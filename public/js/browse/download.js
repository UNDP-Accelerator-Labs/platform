async function setDownloadOptions () {
	...
	const { metafields, modules } = await POST('/load/metadata', { feature: [ 'metafields', 'modules' ] })
	...
	if (['pads', 'mobilizations'].includes(object)) {
		...
		function resetFeatures (data) {
			...
			form.selectAll(`li.opt`)
			.each(function () {
				const sel = d3.select(this)
				const deactivate = !sel.classed(value)
				sel.classed('hide', deactivate)
				sel.selectAll('input:not([type=hidden])').each(function () { this.disabled = deactivate }) // THIS DOES NOT SEEM TO DO ANYTHING
		...
		function downloadPDF (data) {
			const elements = Array.from(this.form.elements).filter(elem => {
				...
				return !parent.classed('hide') && elem.checked && elem.name !== 'output'
			})
			const url = new URL(window.location)
