...
async function addSection (node) {
	const { id, section, singlesection, options } = JSON.parse(d3.select('data[name="pinboard"]').node().value)
	...
	function setSections (data) {
		...
		const platform_filters = JSON.parse(d3.select('data[name="platform-filters"]').node().value)
