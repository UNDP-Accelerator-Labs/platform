const Entry = function (_kwargs) {
	...
	this.data = data
	this.container = parent.addElem('article', `${object.slice(0, -1)}${page.display === 'slideshow' ? ' slide' : ''}`)
		.datum(data)
	.each(function (d) {
	...
	if (page.display !== 'columns') this.metagroup = this.head.addElems('div', 'meta meta-group')
	...
	this.render = {
			...
			metainfo.addElems('div', 'meta meta-date')
				.html(d => {
					if (d.date) {
						...
						if (!(dateobj instanceof Date && !isNaN(dateobj)) && d.date.date) { dateobj = new Date(d.date.date); }
		...
		tags: function (_sel) {
			...
			.addElems('div', 'tag')
				...
				.addElems('label')
				.html(d => {
					if (d.name.length > 25) return `${d.name.slice(0, 25).capitalize()}…`
			...
			if (this.data.editable && (this.data.followups?.length > 0 || this.data.forwards?.length > 0)) {
				...
				if (this.data.forwards?.length) {
					...
					.addElems('a')
						...
						.html(d => {
							if (!mediaSize) var mediaSize = getMediaSize()
							if (mediaSize === 'xs' && d.title.length > 25) return `${d.title.slice(0, 25)}…`
							else return d.title
						})
					.on('click', function (d) {
						const sel = d3.select(this)
		...
		pin: function (_sel) {
			...
			if (
				...
				function renderPins (_group, _data) {
					...
					pin.addElems('label', 'close', d => d.editable ? [d] : [])
					.on('click', async function (d) {
...
async function renderSections () {
	...
	if (!page.mapscale || page.mapscale === 'contain') {
		...
		.each(function (d) {
			...
			// 	section.classed(page.display === 'slideshow' ? page.display : 'columns', true)
			// 	d.data.forEach(c => section.call(renderVignette, { data: c, page }))
			// } else {
			// 	section.classed(page.display, true)
			// 	d.data.forEach(c => section.call(renderVignette, { data: c, page }))
			// }
			section.classed(page.display, true)
			d.data.forEach(c => section.call(renderVignette, { data: c, object, space, page }))
		})

		if (page.display === 'slideshow') { initSlideshow(); }
	}
}
...
async function renderMap () {
	...
	const space = d3.select('data[name="space"]').node().value
	const page = JSON.parse(d3.select('data[name="page"]').node().value)
	let { lnglat: centerpoint } = JSON.parse(d3.select('data[name="location"]').node().value)
	if (!centerpoint) centerpoint = null

	let clusters = await getContent('locations')

	clusters = clusters.filter((cur) => cur !== null)

	// const centerpoint = <%- JSON.stringify(locals.metadata.user?.country?.lnglat || null) %>; // TO DO: GET THIS FOR USER
	// const clusters = <%- JSON.stringify(locals.clusters || []) %>.filter((cur) => cur !== null);
	...
	if (drawMap) {
		...
		if (!clusters.length) {
			...
			async function loadPopup (popup, pads, page = 1) {
				let { data } = await POST(`/${language}/browse/pads/${space}`, { pads, page }) // TO DO: CHANGE TO getContent

				console.log(data)
				// const max_pages = Math.ceil(pads.length / <%- page_content_limit %>)
				const max_pages = Math.ceil(pads.length / data.count)
				...
				const body = d3.select(section)
					...
					.on('scroll', function () {
						...
						const slidewidth = slide.node().clientWidth || slide.node().offsetWidth || slide.node().scrollWidth || 300
						if (this.scrollLeft % slidewidth === 0) {
							idx = Math.round(this.scrollLeft / slidewidth)
							d3.selectAll('.dot').classed('highlight', (d, i) => i === idx)
							d3.selectAll('button.slide-nav').classed('hide', d => {
