...
export function enableNext(node) {
  ...
  const disabled =
    ((node.nodeName === 'TEXTAREA' || node.type === 'text') &&
      node.value.trim().length === 0) ||
    (parent.selectAll('li').node() !== null &&
      parent.selectAll('li.checked').size() === 0);
...
export function togglePadLimit(node) {
  ...
  if (!node.checked) {
    ...
    parent.select('input#hidden-pad-limit').remove();
  }
}
...
export function toggleChecked(node) {
  ...
  parent.select('.contributor-count').html((_) => {
    return `${count} ${
      vocabulary['invited contributors'][count !== 1 ? 'plural' : 'singular']
    }`;
  });
