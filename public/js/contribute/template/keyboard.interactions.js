...
window.addEventListener('keydown', function (e) {
	...
	if (editing) {
		...
		if (
			...
			if (e.key === 'i' || e.keyCode === 73) d3.select('.media-input-group label[for=input-media-img]').classed('highlight', true)
			if (e.key === 'd' || e.keyCode === 68) d3.select('.media-input-group label[for=input-media-drawing]').classed('highlight', true)
			if (e.key === 't' || e.keyCode === 84) d3.select('.media-input-group label[for=input-media-txt]').classed('highlight', true)
			if (e.key === 'l' || e.keyCode === 76) d3.select('.media-input-group label[for=input-media-checklist]').classed('highlight', true)
			if (e.key === 'r' || e.keyCode === 82) d3.select('.media-input-group label[for=input-media-radiolist]').classed('highlight', true)
			if (e.key === 'e' || e.keyCode === 69) d3.select('.media-input-group label[for=input-media-embed]').classed('highlight', true)
		}
	}
})
