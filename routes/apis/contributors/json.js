...
const { checklanguage, safeArr, join, DEFAULT_UUID } =
...
module.exports = async (req, res) => {
  ...
  if (render) {
    ...
    const basedir = path.join(rootpath, '/tmp');
    if (!fs.existsSync(basedir)) fs.mkdirSync(basedir);
    const now = new Date();
    const dir = path.join(basedir, `download-${+now}`);
            ...
            .then((results) => {
              ...
              users.forEach((d) => {
                ...
                if (include_contributions) {
                  ...
                  d.templates =
                    templates?.filter((c) => c.owner === d.uuid) || [];
                  d.campaigns =
                    mobilizations?.filter((c) => c.owner === d.uuid) || [];
                }
              ...
              if (render) {
                fs.writeFileSync(
                  path.join(dir, `${app_title_short}_contributors.json`),
