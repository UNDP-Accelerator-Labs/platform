...
const { checklanguage, array, safeArr, join, DEFAULT_UUID } =
  ...
  DB.general
    .tx((t) => {
      return t
        ...
        ) // NEED TO REMOVE page INFO
        .then(async (users) => {
          // JOIN LOCATION INFO
          users = await join.locations(users, {
            connection: t,
            language,
            key: 'iso3',
          });
            ...
            batch.push(
              DB.conn
                .task((t1) => {
                  ...
                  return t1.batch(batch1).catch((err) => console.log(err));
                })
          ...
          return t
            .batch(batch)
            .then((results) => {
              const [teams, contributions] = results;
              ...
              if (include_data) {
                ...
                const flat_content = users.map((d) => {
                  ...
                  Object.keys(d).forEach((c) => {
                    if (c === 'uuid')
                      obj.contributor_id = `c-${ids.indexOf(d[c]) + 1}`;
                  ...
                  if (single_sheet && include_contributions) {
                    ...
                    for (let i = 0; i < max_templates; i++) {
                      obj[`template_id--${i + 1}`] =
                        user_templates[i]?.id || null;
                      obj[`template_title--${i + 1}`] =
                        user_templates[i]?.title || null;
                    }
                    // ADD PADS TO SINGE SHEET
                    const user_mobilizations = mobilizations.filter(
                      (c) => c.owner === d.uuid,
                    );
                    for (let i = 0; i < max_mobilizations; i++) {
                      obj[`campaign_id--${i + 1}`] =
                        user_mobilizations[i]?.id || null;
                      obj[`campaign_title--${i + 1}`] =
                        user_mobilizations[i]?.title || null;
                    }
                    // TO DO: FILES
                    // TO DO: REVIEWS
                  }
              ...
              if (!single_sheet && include_contributions) {
                ...
                if (pads.length) {
                  ...
                  pads.sort((a, b) =>
                    a.contributor_id.localeCompare(b.contributor_id),
                  );
                ...
                const contributions_sheet = XLSX.utils.json_to_sheet(
                  all_contributions.flat(),
                );
              ...
              if (render) {
                if (single_sheet)
                  XLSX.writeFile(
                    wb,
                    path.join(dir, `${app_title_short}_contributors.csv`),
                    {},
                  );
                else
                  XLSX.writeFile(
                    wb,
                    path.join(dir, `${app_title_short}_contributors.xlsx`),
                    {},
                  );
              }
              return null;
