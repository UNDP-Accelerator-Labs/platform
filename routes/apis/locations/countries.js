...
module.exports = async (req, res) => {
  let { countries, regions, has_lab } = Object.keys(req.query)?.length
    ? req.query
  ...
  let filters = [];
  if (regions)
    filters.push(DB.pgp.as.format('c.bureau IN ($1:csv)', [regions]));
  ...
  DB.general
    .tx(async (t) => {
      const name_column = await geo.adm0.name_column({
        connection: t,
        language,
      });
      ...
      batch.push(
        ...
        ;`,
            [
              name_column,
              countries
                ? DB.pgp.as.format('AND su_a3 IN ($1:csv)', [countries])
                : '',
            ],
          )
      ...
      batch.push(
        ...
        ;`,
            [
              name_column,
              countries
                ? DB.pgp.as.format('AND adm_a3 IN ($1:csv)', [countries])
                : '',
            ],
          )
          .catch((err) => console.log(err)),
      ...
      return t
        ...
        .then((results) => {
          ...
          const locations = join.concatunique.call(su_a3, [
            adm_a3,
            'country',
            'latter',
          ]);
