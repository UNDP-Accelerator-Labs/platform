...
const { app_title_short, app_storage, colors, metafields, DB } =
  include('config/');
const { checklanguage, datastructures, parsers, array, join, geo } =
    ...
    .then(async (results) => {
                    ...
                    const items = await Promise.all(
                      c.sections.map(async (b) => {
                        ...
                        if (b.title || b.lead) {
                          ...
                          if (b.lead)
                            paragraphs.push(new Paragraph({ text: b.lead }));
                        }
                        if (b.items?.length) {
                          const items = await Promise.all(
                            b.items.map((a) => populateSection(a)),
                          );

                          // paragraphs.push(...b.items.map(a => populateSection(a)).filter(a => a.length).flat())
                          paragraphs.push(
                            ...items.filter((a) => a.length).flat(),
                          );
                        }
                        return paragraphs;
                      }),
      ...
      async function populateSection(data, repetition = 0) {
        return new Promise(async (resolve) => {
          ...
          if (instruction) {
            ...
            if (level === 'meta') {
              arr.push(
                new Paragraph({
                  text: capitalize(
                    metafields.find((c) => c.label === name)?.name || name,
                  ),
                  heading: HeadingLevel.HEADING_4,
                }),
              );
            ...
            if (app_storage) {
              ...
              if (src) {
                ...
                await new Promise((resolve1) => {
                  request.get(
                    new URL(
                      path.join(new URL(app_storage).pathname, src),
                      app_storage,
                    ).href,
              ...
              await Promise.all(
                srcs.map((d) => {
                  return new Promise((resolve1) => {
                    request.get(
                      new URL(
                        path.join(new URL(app_storage).pathname, d),
                        app_storage,
                      ).href,
                      function (err, res, buffer) {
                        if (err) console.log(err);
          ...
          if (type === 'embed')
            arr.push(new Paragraph({ text: data.html, style: 'main' })); // TO DO: MIGHT NEED TO CLEAN THE HTML
          ...
          if (['checklist', 'radiolist'].includes(type)) {
            ...
            options.forEach((d, i) => {
              const children = [
                new TextRun({ text: d.name, bold: d.checked ? true : false }),
              ];
        ...
        const toc = {
          children: [
            new TableOfContents('Summary', {
              ...
              stylesWithLevels: [
                new StyleLevel('toc', 1),
                new StyleLevel('toc', 2),
                new StyleLevel('toc', 3),
              ], // THIS IS NOT WORKING
        ...
        Packer.toBuffer(doc)
          .then((buffer) => {
            ...
            if (open) {
              ...
              const zip = spawn(
                'zip',
                ['-P', pw, 'archive.zip', `${app_title_short}.docx`],
                { cwd: basedir },
              );
