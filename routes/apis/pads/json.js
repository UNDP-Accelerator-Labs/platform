...
module.exports = async (req, res) => {
  ...
  if (typeof include_imgs === 'string')
    include_imgs = JSON.parse(include_imgs);
      ...
      pads = await Promise.all(
        pads.map((pad_group) => {
          return new Promise(async (resolve) => {
            ...
            pad_group.values = await Promise.all(
              pad_group.values.map((d) => {
                return new Promise(async (resolve) => {
                  ...
                  if (include_imgs) {
                    d.media = d.media?.map((c) => {
                      if (parsers.isURL(c)) return c;
                        ...
                        if (render) {
                          // && fs.existsSync(path.join(rootpath, `/public${c.replace('uploads/sm', 'uploads')}`))) {
                          return `images/pad-${d.pad_id}/image-${
                            idx + 1
                          }${path.extname(c)}`;
                        } else {
                          if (app_storage) {
                            ...
                            return new URL(
                              path.join(new URL(app_storage).pathname, c),
                              app_storage,
                            ).href;
                  ...
                  if (include_comments) {
                    let commenter_list = array.unique.call(
                      pad_group.values.map((d) => d.comments).flat(),
                      { key: 'user_id', onkey: true },
                    );
            ...
            if (output === 'geojson') {
              pad_group.values = pad_group.values
                .map((d) => {
            ...
            if (render) {
              if (use_templates) {
                ...
                if (include_imgs && imgs.length > 0) {
                  ...
                  if (app_storage) {
                    ...
                    const containerClient =
                      blobServiceClient.getContainerClient(app_title_short);
                        ...
                        return new Promise(async (resolve1) => {
                          ...
                          try {
                            const blobClient = containerClient.getBlobClient(
                              `${d.image.replace('/uploads/sm', 'uploads')}`,
                            );
                ...
                fs.writeFileSync(
                  path.join(template_dir, 'data.json'),
                  JSON.stringify(pad_group.values),
                );
              } else {
                if (include_imgs && imgs.length > 0) {
                ...
                fs.writeFileSync(
                  path.join(dir, `${app_title_short}_data.json`),
                  JSON.stringify(pad_group.values),
                );
              }
            }
