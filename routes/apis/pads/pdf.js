...
exports.populateSection = (data, doc, inset = false) => {
	...
	return new Promise(async resolve => {
		...
		// META
		if (data.type === 'location') this.addLocations(data, doc, inset)
...
exports.addInstruction = (data, doc, inset = false) => {
	...
	if (instruction) {
		doc.fillColor(colors['light-grey'])
...
exports.addTitle = (data, doc, inset = false) => {
	...
	doc.fillColor(colors['dark-blue'])
		.fontSize(t_xlarge)
		.text(txt)
		.fillColor(colors['dark-grey']) // RESET
	doc.moveDown()
...
exports.addImg = async (data, doc, inset = false) => {
	...
	if (src) {
		if (scale === 'fitted') doc.image(src, doc.x, doc.y, { width: imgwidth, height: imgheight })
		else if (scale === 'original') doc.image(src, doc.x, doc.y, { fit: [ maxwidth, imgheight ], align: textalign })
	}
}
exports.addMosaic = async (data, doc, inset = false) => {
	...
	const maxwidth = ((width - margins.left - margins.right - (colgap * (cols - 1))) / cols)
	const maxheight = height - margins.top - margins.bottom
	...
	for (let i = 0; i < chunks.length; i ++) {
		...
		row.forEach(d => {
			...
			if (src) {
				if (scale === 'fitted') doc.image(src, x, y, { fit: [ width, offsetheight], valign: verticalalign })
				else if (scale === 'original') doc.image(src, x, y, { fit: [ maxwidth, height ], valign: verticalalign })
			}
		})
...
exports.addChecklist = (data, doc, inset = false) => {
	...
	options.forEach(d => {
		doc//.font('fonts/Palatino.ttf')
			...
			.text(d.name, inset ? margins.left * 1.5 : margins.left, doc.y, { strike: !d.checked })
			.fillColor(colors['dark-grey']) // RESET
...
exports.addLocations = (data, doc, inset = false) => {
	// TO DO: FINISH THIS
...
exports.addIndexes = async (data, doc, inset = false) => {
	...
	const srcs = tags.map(d => `./public/imgs/sdgs/${lang}/G${d.key || d}-c.png`)
	...
	const imgwidth = ((width - margins.left - margins.right - (colgap * (cols - 1))) / cols)
	const maxheight = height - margins.top - margins.bottom
