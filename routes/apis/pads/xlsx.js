...
const { app_title_short, app_storage, metafields, media_value_keys, DB } =
  include('config/');
const { checklanguage, array, join, parsers, flatObj, safeArr } =
...
module.exports = async (req, res) => {
  ...
  let {
    ...
    ? req.body
    : {};
  if (typeof use_templates === 'string')
    use_templates = JSON.parse(use_templates);
  if (typeof include_data === 'string')
    include_data = JSON.parse(include_data);
  if (typeof include_locations === 'string')
    include_locations = JSON.parse(include_locations);
    ...
    .tx((t) => {
      return t
        ...
        .then(async (pads) => {
          ...
          let contributor_list = array.unique.call(pads, {
            key: 'contributor_id',
            onkey: true,
          });
          ...
          const batch = pads.map((pad_group) => {
            return t
              .task((t1) => {
                const batch1 = [];
                const ids = safeArr(
                  pad_group.values.map((d) => d.pad_id),
                  -1,
                );
                if (include_tags) {
                  batch1.push(
                        ...
                        const tags = await Promise.all(
                          nest.map((d) => {
                            return new Promise(async (resolve) => {
                              const tags = await join.tags(d.values, [
                                language,
                                'tag_id',
                                d.key,
                              ]);
                ...
                return t1
                  .batch(batch1)
                  .then(async (results) => {
                    const [tags, locations, metadata, engagement, comments] =
                      results;
                    ...
                    if (include_data) {
                      ...
                      if (include_imgs) {
                        ...
                        var max_imgs = Math.max(
                          ...pad_group.values.map((d) => d.img?.length ?? 0),
                        );

                        var imgs = pad_group.values
                          .map((d) => {
                            return d.img?.map((c) => {
                              const obj = {};
                              obj.pad_id = d.pad_id;
                      ...
                      if (include_tags) {
                        ...
                        const tag_types = array.unique.call(tags, {
                          key: 'type',
                          onkey: true,
                        });
                        const tag_counts = array.nest
                          .call(tags, { key: 'pad_id' })
                          .map((d) => {
                      ...
                      if (include_locations) {
                        var max_locations = array.nest
                          .call(locations, { key: 'pad_id' })
                          ?.map((d) => d.count);
                        if (max_locations.length)
                          max_locations = Math.max(...max_locations);
                        else max_locations = 0;
                      }
                      // EXTRACT METAFIELDS AND DETERMINE MAX METAFIELDS
                      if (include_metafields) {
                        const meta_types = array.unique.call(metadata, {
                          key: (d) => `${d.type}-${d.name}`,
                          onkey: true,
                        });
                        const meta_counts = array.nest
                          .call(metadata, { key: 'pad_id' })
                          .map((d) => {
                            return array.nest.call(d.values, {
                              key: (c) => `${c.type}-${c.name}`,
                            });
                          })
                          .flat();
                      ...
                      if (use_templates) {
                        ...
                        function create_id(d, id_prefix, name_prefix) {
                          if (Array.isArray(d)) {
                            ...
                            var id = `${id_prefix ? `${id_prefix}--` : ''}${
                              d.type ?? undefined
                            }-${d.title ?? undefined}-${d.lead ?? undefined}`;
                            var name = `${
                            ...
                            var id = `${id_prefix ? `${id_prefix}--` : ''}${
                              d.level ?? undefined
                            }-${d.type ?? undefined}-${d.name ?? undefined}-${
                              d.instruction?.length ? d.instruction : undefined
                            }`;
                            var name = `${
                              name_prefix ? `${name_prefix}--` : ''
                            }${
                            ...
                            if (
                              ...
                              if (d.type === 'checklist') {
                                d.options?.forEach((c) => {
                                  const obj = {};
                                  obj.id = `${cid}--${c.name}`;
                                  obj.repetition = structure.some(
                                    (c) => c.id === obj.id,
                                  )
                                    ? structure.filter((c) => c.id === obj.id)
                                        .length
                                    : 0;
                                  obj.name = `${
                                    obj.repetition > 0
                                ...
                                for (let i = 0; i < max_locations; i++) {
                                  ...
                                  if (d.centerpoints?.[i])
                                    obj.content = `${d.centerpoints[i]?.lat}, ${d.centerpoints[i]?.lng}`;
                                  else obj.content = null;
                                  structure.push(obj);
                                }
                              } else if (
                                include_tags &&
                                ['index', 'tag'].includes(d.type)
                              ) {
                                const max =
                                  max_tags.find((c) => c.type === d.name)
                                    ?.max ?? 0;
                                ...
                                metafields
                                  .filter(
                                    (c) =>
                                      !['tag', 'index', 'location'].includes(
                                        c.type,
                                      ),
                                  )
                                  .some(
                                    (c) =>
                                      c.type === d.type && c.name === d.name,
                                  )
                              ) {
                                const max =
                                  max_metafields.find(
                                    (c) => c.type === `${d.type}-${d.name}`,
                                  )?.max ?? 0;
                                ...
                                if (include_imgs && d.type === 'img') {
                                  if (d.src) {
                                    ...
                                    if (
                                      ...
                                      obj.content = `images/pad-${pad_id}/image-${
                                        idx + 1
                                      }${path.extname(d.src)}`;
                                    ...
                                    obj.content = d.srcs
                                      .map((c) => {
                                        ...
                                        if (
                                          ...
                                          return `images/pad-${pad_id}/image-${
                                            idx + 1
                                          }${path.extname(c)}`;
                                        } else return c;
                                      })
                                ...
                                if (include_imgs && d.type === 'video')
                                  obj.content = d.src;
                                if (d.type === 'drawing')
                                  obj.content = d.shapes?.join(', ');
                                if (d.type === 'txt') obj.content = d.txt;
                                if (d.type === 'embed')
                                  obj.content =
                                    d.html?.replace(/<[^>]*>/g, '') || d.src; // THE replace IS IMPORTANT HERE TO AVOID xml INJECTION IN THE xlsx OUTPUT
                        ...
                        var flat_content = pad_group.values.map((d) => {
                          const structure = check_for_items(
                            d.sections,
                            d.pad_id,
                          ).map((c) => {
                            const obj = {};
                          ...
                          return Object.assign(flatObj.call(structure), {
                            pad_id: d.pad_id,
                          });
                        });

                        const content_lengths = flat_content.map(
                          (d) => Object.keys(d).length,
                        );
                        var headers = flat_content.find(
                          (d) =>
                            Object.keys(d).length ===
                            Math.max(...content_lengths),
                        );
                      ...
                      pad_group.values.forEach((d) => {
                        ...
                        d.contributor_id = `c-${
                          contributor_list.indexOf(d.contributor_id) + 1
                        }`;
                        ...
                        if (!use_templates) {
                          ...
                          const structure = flat_content.find(
                            (c) => c.pad_id === d.pad_id,
                          );
                          headers.forEach((c) => {
                            ...
                            for (let i = 0; i < max_imgs; i++) {
                              if (d.img[i]) {
                                ...
                                if (
                                  ...
                                  d[`media-${i + 1}`] = `images/pad-${
                                    d.pad_id
                                  }/image-${idx + 1}${path.extname(d.img[i])}`;
                        ...
                        if (single_sheet && include_locations) {
                          const pad_locations = locations.filter(
                            (c) => c.pad_id === d.pad_id,
                          );
                          for (let i = 0; i < max_locations; i++) {
                            if (pad_locations[i]) {
                      ...
                      const data_sheet = XLSX.utils.json_to_sheet(
                        pad_group.values,
                      );
                        ...
                        if (imgs?.length) {
                          const imgs_data = imgs.map((d, i) => {
                            ...
                            if (
                              ...
                              obj.image = `images/pad-${d.pad_id}/image-${
                                i + 1
                              }${path.extname(d.image)}`;
                            else obj.image = d.image;
                            return obj;
                          });
                          const imgs_sheet =
                            XLSX.utils.json_to_sheet(imgs_data);
                          XLSX.utils.book_append_sheet(
                            wb,
                            imgs_sheet,
                            'data-media',
                    ...
                    if (!single_sheet && tags?.length) {
                      ...
                      tags.forEach((d) => {
                        d.type =
                          metafields.find((c) => c.label === d.type)?.name ??
                          d.type;
                      });
                    ...
                    if (!single_sheet && engagement?.length) {
                      ...
                      const engagement_sheet =
                        XLSX.utils.json_to_sheet(engagement);
                      XLSX.utils.book_append_sheet(
                        wb,
                        engagement_sheet,
                        'metadata-engagement',
                    ...
                    if (!single_sheet && comments?.length) {
                      ...
                      let commenter_list = array.unique.call(comments, {
                        key: 'user_id',
                        onkey: true,
                      });
                      ...
                      comments.forEach((d) => {
                        // ANONYMIZE CONTRIBUTORS
                        d.user_id = `u-${
                          commenter_list.indexOf(d.user_id) + 1
                        }`;
                      });
                    ...
                    if (render) {
                      if (use_templates) {
                        const template_dir = path.join(
                          dir,
                          `${app_title_short}_template_${pad_group.key}`,
                        );
                        ...
                        if (include_imgs && imgs.length > 0) {
                          ...
                          if (app_storage) {
                            // A CLOUD BASED STORAGE OPTION IS AVAILABLE
                            ...
                            await Promise.all(
                              imgs.map((d, i) => {
                                return new Promise(async (resolve1) => {
                                  const img_pad_dir = path.join(
                                    img_dir,
                                    `pad-${d.pad_id}`,
                                  );
                                  if (!fs.existsSync(img_pad_dir))
                                    fs.mkdirSync(img_pad_dir);
                            ...
                            imgs.forEach((d, i) => {
                              ...
                              try {
                                fs.copyFileSync(
                                  ...
                                  path.join(
                                    img_pad_dir,
                                    `image-${i + 1}${path.extname(d.image)}`,
                                  ),
                        ...
                        if (single_sheet)
                          XLSX.writeFile(
                            wb,
                            path.join(template_dir, 'data.csv'),
                            {},
                          );
                        else
                          XLSX.writeFile(
                            wb,
                            path.join(template_dir, 'data.xlsx'),
                            {},
                          );
                      } else {
                        if (include_imgs && imgs.length > 0) {
                        ...
                        if (single_sheet)
                          XLSX.writeFile(
                            wb,
                            path.join(dir, `${app_title_short}_data.csv`),
                            {},
                          );
                        else
                          XLSX.writeFile(
                            wb,
                            path.join(dir, `${app_title_short}_data.xlsx`),
                            {},
                          );
                      }
                    }
    ...
    .then((results) => {
      ...
      if (render) {
        if (open)
          var zip = spawn(
            'zip',
            ['-r', 'archive.zip', path.relative(basedir, dir)],
            { cwd: basedir },
          );
        else
          var zip = spawn(
            'zip',
            ['-rP', pw, 'archive.zip', path.relative(basedir, dir)],
            { cwd: basedir },
          );
        ...
        if (data.length) res.send(data[0].replace(/\n/g, '<br/>'));
          ...
          res.send(
            'Sorry you do not have the rights to download this content. Please enquire about getting an access token to view download this content.',
          );
      }
    })
    .catch((err) => console.log(err));
