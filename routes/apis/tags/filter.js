...
module.exports = async (req, res) => {
  let { tags, type, pads, mobilizations, countries, regions, timeseries } =
  ...
  if (mobilizations && !Array.isArray(mobilizations))
  ...
  return new Promise(async (resolve) => {
    ...
    if (tags) {
      ...
      if (timeseries)
        platform_filters.push(
          DB.pgp.as.format(`t.tag_id IN ($1:csv)`, [tags]),
        );
    }
    if (pads)
      platform_filters.push(DB.pgp.as.format(`t.pad IN ($1:csv)`, [pads]));
    if (mobilizations)
      platform_filters.push(
        DB.pgp.as.format(
          `t.pad IN (SELECT pad FROM mobilization_contributions WHERE mobilization IN ($1:csv))`,
          [mobilizations],
        ),
      );
    if (countries) {
      if (metafields.some((d) => d.type === 'location')) {
        platform_filters.push(
          DB.pgp.as.format(
            `t.pad IN (SELECT pad FROM locations WHERE iso3 IN ($1:csv))`,
            [countries],
          ),
        );
      } else {
        platform_filters.push(
          await DB.general
            ...
            .then((results) =>
              DB.pgp.as.format(
                `t.pad IN (SELECT id FROM pads WHERE owner IN ($1:csv))`,
                [
                  safeArr(
                    results.map((d) => d.uuid),
                    DEFAULT_UUID,
                  ),
                ],
              ),
    ...
    if (general_filters.length && general_filters.slice(0, 3) !== 'AND')
      general_filters = `AND ${general_filters}`;
    if (platform_filters.length && platform_filters.slice(0, 3) !== 'AND')
      platform_filters = `AND ${platform_filters}`;
