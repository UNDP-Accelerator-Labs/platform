...
module.exports = async (req, res) => {
  let { timeseries, aggregation } = Object.keys(req.query)?.length
    ? req.query
    ...
    DB.conn
      ...
      .then((results) => {
        if (results.length) {
            ...
            .then((tags) => {
              ...
              if (!timeseries)
                ...
                data.forEach((d) => {
                  ...
                  d.timeseries = array.count.call(
                    d.dates.map((c) => c.getTime()),
                    { keyname: 'date' },
                  );
          ...
          res
            .status(400)
            .json({ message: 'Sorry it seems there is no content here.' });
      })
      .catch((err) => console.log(err));
  } else {
    DB.general
