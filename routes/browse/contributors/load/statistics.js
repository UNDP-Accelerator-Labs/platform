...
exports.main = kwargs => {
	...
	return conn.task(t => {
		...
		batch.push(t.any(`
			...
			GROUP BY status
			ORDER BY status
		;`, [ uuid, rights ]).then(d => { return { total: d } }))
		...
		batch.push(t.any(`
			...
			GROUP BY status
			ORDER BY status
		;`, [ uuid, rights, full_filters.replace(`AND LEFT(u.name, 1) = '${page}'`, '') ]).then(d => { return { filtered: d } })) // WE NEED TO REMOVE THE PAGE FILTER
		...
		batch.push(t.any(`
			...
			GROUP BY status
			ORDER BY status
		;`, [ uuid, rights, full_filters ]).then(d => { return { displayed: d } }))
		...
		batch.push(t.any(`
			...
			GROUP BY status
			ORDER BY status
		;`, [ uuid, rights, full_filters.replace(`AND LEFT(u.name, 1) = '${page}'`, '').replace(/AND u.confirmed::INT IN \([\'\d\,\s]+\)/g, '') ]).then(d => { return { persistent: d } })) // WE NEED TO REMOVE THE PAGE FILTER
		...
		batch.push(t.any(`
			...
			WHERE uuid IN (SELECT contributor FROM cohorts WHERE host = $1)
				OR $2::INT > 2
			GROUP BY initial
		;`, [ uuid, rights ]).then(d => { return { toc: d } }))
