...
exports.count = function (kwargs = {}) {
  // THIS IS A SIMPLIFIED VERSION OF nest WHERE WE DO NOT KEEP THE nested DATA
...
exports.unique = function (kwargs = {}) {
  ...
  this.forEach((d) => {
    if (!key) {
      ...
      const groupby = typeof key === 'function' ? key(d) : d[key];
      if (onkey) {
        if (!arr.map((c) => c).includes(groupby)) arr.push(groupby);
      } else {
        if (!arr.map((c) => c[key]).includes(groupby)) arr.push(d);
      }
    }
  });
  return arr;
