...
exports.sessionsummary = async (_kwargs) => {
  ...
  return new Promise((resolve) => {
    if (uuid) {
      conn
        .manyOrNone(`SELECT sess FROM session WHERE sess ->> 'uuid' = $1;`, [
          uuid,
        ])
        .then((sessions) => {
          if (sessions) {
...
exports.pagemetadata = (_kwargs) => {
  ...
  const { page, pagecount, map, display, mscale, excerpt, req, res } =
    _kwargs || {};
  ...
  const language = checklanguage(
    params?.language || session.language || this.sessiondata(),
  );
  ...
  return conn
    .tx((t) => {
      ...
      // TEMPLATE LIST
      if (
        modules.some((d) => d.type === 'templates' && rights >= d.rights.read)
      ) {
        batch.push(
          t
            .any(
      ...
      if (
        modules.some(
          (d) => d.type === 'mobilizations' && rights >= d.rights.read,
        )
      ) {
        batch.push(
          t
      ...
      batch.push(
        DB.general
          .task((gt) => {
            ...
            return gt.batch(gbatch).catch((err) => console.log(err));
          })
          .catch((err) => console.log(err)),
      );
      // REVIEW TEMPLATES
      if (
        modules.some((d) => d.type === 'reviews' && rights >= d.rights.read)
      ) {
        batch.push(
          t.any(`
              ...
              return pinboard_stats.map((stats) => {
                return {
                  ...
                  contributors: new Set(
                    pinpads.get(stats.id).map((pad_id) => owners.get(pad_id)),
                  ).size,
                };
              });
            })
    ...
    .then(async (results) => {
      ...
      if (review_templates) {
        ...
        review_templates = join.multijoin.call(review_templates, [
          speakers,
          'language',
        ]);
        review_templates = join.multijoin.call(review_templates, [
        ...
        review_templates.forEach((d) => {
          d.disabled =
            d.count <
            (modules.find((d) => d.type === 'reviews')?.reviewers ?? 0);
        });
      ...
      obj.metadata = {
        ...
        page: {
          ...
          public,
          excerpt: excerpt || {
            title: res?.locals.instance_vars?.title || title,
            txt: res?.locals.instance_vars?.description || description,
            p: false,
          },
...
exports.legacy.publishablepad = (_kwargs) => {
  // THIS IS LEGACY FOR THE SOLUTIONS MAPPING PLATFORM
  const conn = _kwargs.connection || DB.conn;
  const { data } = _kwargs;

  const other_metadata = metafields.filter(
    (d) => !['tag', 'index', 'location'].includes(d.type),
  );
      ...
      return Promise.all(
        data.map((d) => {
          return conn
            ...
            .then((meta) => {
              const nesting = array.nest.call(meta, {
                key: (c) => `${c.type}-${c.name}`,
                keep: ['type', 'name'],
              });
              const has_metadata = other_metadata.every((c) =>
                nesting.some(
                  (b) =>
                    c.required &&
                    b.type === c.type &&
                    b.name === c.name &&
                    b.count <= (c.limit ?? Infinity),
                ),
              );
