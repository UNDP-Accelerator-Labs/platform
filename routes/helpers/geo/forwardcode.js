    ...
    return new Promise((resolve) => {
      if (!l || typeof l !== 'string') {
        ...
        setTimeout((_) => {
          ...
          fetch(
            `https://api.opencagedata.com/geocode/v1/json?q=${l_formatted}&key=${process.env.OPENCAGE_API}`,
          )
            ...
            .then((data) => {
              ...
              if (data.results.length) {
                ...
                obj.found = true;
                obj.caption = `<strong>${l
                  .trim()
                  .capitalize()}</strong> found using <a href='https://opencagedata.com/credits' target='_blank'>OpenCage Geocoder</a> | &copy; <a href='https://www.openstreetmap.org/copyright' target='_blank'>OpenStreetMap contributors</a>`; // TO DO: TRANSLATE
              } else {
                obj.found = false;
                obj.centerpoint = centerpoint;
                obj.caption = `No location was found for <strong>${l
                  .trim()
                  .capitalize()}</strong>.`; // TO DO: TRANSLATE
              }
              resolve(obj);
            })
...
exports.render = (req, res) => {
  ...
  Promise.all(promises)
    ...
    .catch((err) => {
      ...
      res.json({
        status: 500,
        message: 'Oops! Something went wrong while searching for locations.',
      });
    });
};
