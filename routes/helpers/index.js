...
exports.engagementsummary = function (kwargs = {}) {
  ...
  if (engagementtypes.includes('comment')) {
    ...
    coalesce.push(
      DB.pgp.as.format(`COALESCE (ce.$1:name, 0)::INT AS $1:name`, [
        'comments',
      ]),
    );
    ...
    query.push(
      DB.pgp.as.format('SELECT e.docid$1:raw', [
        engagementtypes.filter((e) => e !== 'comment').length > 0 ? ',' : '',
      ]),
    );
  ...
  engagementtypes
    ...
    .forEach((e, i) => {
      query.push(
        DB.pgp.as.format(
          `SUM (CASE WHEN e.type = $1 THEN 1 ELSE 0 END) AS $2:name$3:raw`,
          [
            e,
            `${e}s`,
            i < engagementtypes.filter((e) => e !== 'comment').length - 1
              ? ','
              : '',
          ],
        ),
        ...
        cases.push(
          DB.pgp.as.format(
            ...
            [
              ...
              doctype,
              uuid,
              e.charAt(e.length - 1) !== 'e' ? `${e}ed` : `${e}d`,
            ],
          ),
        );
      } else
        cases.push(
          DB.pgp.as.format(`FALSE AS $1:name`, [
            e.charAt(e.length - 1) !== 'e' ? `${e}ed` : `${e}d`,
          ]),
        );
  ...
  if (engagementtypes.filter((e) => e !== 'comment').length > 0)
    query.push('FROM engagement e');
  if (engagementtypes.includes('comment')) {
    query.push(
      DB.pgp.as.format(
        `
  ...
  let comment_clause = '';
  if (engagementtypes.includes('comment'))
    comment_clause = DB.pgp.as.format(' OR cnt.doctype = $1', [doctype]);
  if (docid)
    query.push(
      DB.pgp.as.format(
        `WHERE (e.doctype = $1$3:raw) AND e.docid = $2 GROUP BY (e.docid, cnt.docid, cnt.count)`,
        [doctype, docid, comment_clause],
      ),
    );
  else
    query.push(
      DB.pgp.as.format(
        `WHERE (e.doctype = $1$2:raw) GROUP BY (e.docid, cnt.docid, cnt.count)`,
        [doctype, comment_clause],
      ),
    );
