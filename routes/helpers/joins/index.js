...
exports.users = (data, args = []) => {
  ...
  if ((Array.isArray(data) && data.length) || data?.[key]) {
    const uuids = Array.isArray(data)
      ? [...new Set(data.map((d) => d[key]))]
      : data[key];

    return DB.general
      .tx(async (t) => {
        const name_column = await adm0.name_column({
          connection: t,
          language,
        });
        ...
        return t
          ...
          .then((users) => {
            if (Array.isArray(data))
              return this.multijoin.call(data, [users, key]);
...
exports.tags = (data, args = []) => {
  const [lang, key, tagname, _tagtype] = args;
  ...
  if (data?.length) {
    return DB.general
      .tx((t) => {
        return t
          .any(
            `SELECT id AS $1:name, key, language FROM tags WHERE id IN ($2:csv);`,
          ...
          .then((results) => {
            ...
            const tags_with_equivalences = data.filter(
              (d) => ![undefined, null].includes(d.key),
            );
            const tags_without_equivalences = data.filter((d) =>
              [undefined, null].includes(d.key),
            );
                ...
                t
                  .any(
                    ...
                    [
                      key,
                      tagname,
                      tags_with_equivalences.map((d) => d.key),
                      lang,
                    ],
                  )
                  .then((tags) => {
                    // data = this.multijoin.call(data, [ results, key ])
                    return this.multijoin.call(tags_with_equivalences, [
                      tags,
                      'key',
                    ]);
                    // return { key: 'key', tags }
                ...
                t
                  .any(
                    ...
                    [
                      key,
                      tagname,
                      tags_without_equivalences.map((d) => d[key]),
                      lang,
                    ],
                  )
                  .then((tags) => {
                    return this.multijoin.call(tags_without_equivalences, [
                      tags,
                      key,
                    ]);
                    // return { key, tags }
  ...
  this.forEach((d) => {
    if (key) {
      if (arrcopy.some((c) => c[key] === d[key]) && keep === 'latter') {
        const duplicate = arrcopy.splice(
          arrcopy.findIndex((c) => c[key] === d[key]),
          1,
        )[0];
...
exports.locations = (data, kwargs = {}) => {
  ...
  if ((Array.isArray(data) && data.length) || data?.[key]) {
    const iso3s = Array.isArray(data)
      ? [...new Set(data.map((d) => d[key]))]
      : data[key];
      ...
      .task(async (t) => {
            ...
            .any(
              `
                ...
                WHERE su_a3 IN ($5:csv)
                    AND su_a3 <> adm0_a3
            ;`,
              [key, name_column, name_key, location_structure, iso3s],
            )
            .catch((err) => console.log(err)),
        );
        ...
        return t
          ...
          .then((results) => {
            ...
            if (Array.isArray(data))
              return this.multijoin.call(data, [locations, key]);
