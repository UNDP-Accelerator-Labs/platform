exports.getImg = (_json = {}, _unique = true) => {
  if (_json?.sections) {
    const media = _json.sections
      .map((c) =>
        c.items?.map((b) => (b.type === 'group' ? b.items.flat() : b)).flat(),
      )
      .flat();
    const img = media
      .filter((c) => c.type === 'img' && c.src)
      .map((d) => {
        if (this.isURL(d.src)) return d.src;
        else return `/${d.src.replace('uploads', 'uploads/sm')}`;
      });
    const mosaic = media
      .filter((c) => c.type === 'mosaic' && c.srcs?.length > 0)
      .map((d) => d.srcs)
      .flat()
      .map((d) => {
...
exports.getSDGs = function (_json = {}) {
  if (_json?.sections) {
    const meta = _json.sections
      .map((c) => c.items?.map((b) => (b.type === 'group' ? b.items : b)))
      .flat(3);
...
exports.getTxt = function (_json = {}) {
  if (_json?.sections) {
    const media = _json.sections
      ...
      .flat(3);
    let txt = media.find(
      (c) => c.type === 'txt' && c.is_excerpt && c.txt?.length,
    );
    if (!txt)
      txt = media.find(
        (c) =>
          c.type === 'txt' &&
          c.txt?.length &&
          c.txt?.trim().toLowerCase() !== _json?.title?.trim().toLowerCase(),
      );
...
exports.getReviewScore = function (_json = {}) {
  if (_json?.sections) {
    const media = _json.sections
      .map((d) => d.items?.map((c) => (c.type === 'group' ? c.items : c)))
      .flat(3);
    const score = media
      .find((d) => d.type === 'radiolist' && d.name === 'review_score')
      ?.options.find((d) => d.checked)?.value;
    return score ?? null;
  } else return null;
};
exports.getAttachments = (_json = {}) => {
  if (_json?.sections) {
    ...
    const attachments = meta
      .filter(
        (c) => c.type === 'attachment' && c.srcs?.filter((b) => b).length > 0,
      )
      .map((d) => d.srcs)
      .flat();
    // .map(d => {
