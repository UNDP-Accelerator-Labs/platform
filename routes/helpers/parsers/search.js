...
function completeStr(_str) {
  ...
  function helper(_i = 0) {
    ...
    if (!/\bor\b/i.test(behind[behind.length - 2])) {
      if (
        behind[behind.length - 1].charAt(0) !== '(' &&
        behind[behind.length - 1].charAt(
          behind[behind.length - 1].length - 1,
        ) !== ')'
      ) {
    ...
    if (addParentheses === true) {
      if (!/\bor\b/i.test(forward[1])) {
        if (
          ...
          const insertidx = [..._str.matchAll(/\)/dg)].find(
            (d) => d.index >= end + forward[0].length,
          ).index;

          const parenthesesgroup = _str.slice(end, insertidx + 1).split(/\s/);
          if (!parenthesesgroup.find((d) => /\bor\b/i.test(d))) {
            _str = `${_str.slice(0, end)}${parenthesesgroup
              .join(' ')
              .replace(/[\(\)]/g, '')})${_str.slice(
              end + parenthesesgroup.join(' ').length,
            )}`;
          } else {
            _str = `${_str.slice(0, insertidx)})${_str.slice(insertidx)}`;
          }
        ...
        if (idx === forward.length - 1) {
          ...
          if (allORs) {
            _str = `${_str.slice(0, end)}${forward
              ...
              .join(' ')
              .replace(/[\(\)]/g, '')})${_str.slice(
              end + forward.slice(0, idx + 1).join(' ').length,
            )}`;
          } else {
            if (
              forward[0].charAt(0) === '(' &&
              forward[idx].charAt(forward[idx].length - 1) === ')'
            ) {
              _str = `${_str.slice(0, end)}${forward
                .slice(0, idx + 1)
                .join(' ')
...
function findNests(_str) {
    ...
    return nests.reverse().map((d) => {
      ...
      if (fNest.test(inner)) {
        ...
        innernests.reverse().forEach((c) => {
          ...
          inner = replacement
            .slice(replacement.indexOf('(') + 1, replacement.length - 1)
            .trim();
        });
      }

      let str = '';
      if (/\bOR\b/gi.test(inner)) {
        // JOIN OR STATEMENT
        if (!/\bOR\b/gi.test(outer)) {
          str = inner
            .split(/\bOR\b/gi)
            .map((c) => `${outer} ${c.trim()}`.trim())
            .join(' OR ')
            .trim();
        } else {
  ...
  uniqueterms.reverse().forEach((d) => {
    ...
    if (openParentheses === closeParentheses)
      _str = `${_str.slice(0, d.index)}(${d[0]})${_str.slice(
        d.index + d[0].length,
      )}`;
  });
...
exports.sqlregex = (_data, _language) => {
  // DO SOMETHING HERE FOR RTL LANGUAGES
  ...
  const terms = resetQuotes(
    cartesian(jsonify(findNests(completeStr(prepStr(_data))))),
  );
