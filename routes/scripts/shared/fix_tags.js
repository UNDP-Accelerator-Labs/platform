...
DB.conn.tx(async t => {
	await DB.general.tx(async gt => {
		...
		let [ notype_tags, type_tags ] = await gt.batch(gbatch)
		notype_tags = notype_tags.map(d => d.id)

		let results = await t.any(`
						...
						.forEach(b => {
							...
							if (unregistered_tags.length > 0) {
								unregistered_tags.forEach(async a => {
									...
									// IF THE TAG IS NOT ALREADY IN THE DATABASE, STORE IT TO SAVE LATER
									if (!exists_in_db) {
